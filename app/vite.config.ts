...
function parseRepoModels(repoPath: string) {
...
function readModelRecords() {
  const modelFile = path.join(modelsRoot, 'model-registry.json')
  const payload = safeReadJson(modelFile)
  if (Array.isArray(payload)) return payload
  if (payload && Array.isArray((payload as { records?: unknown[] }).records)) {
...
function mapProviderCode(provider: string) {
  ...
  return map[provider] ?? 'chatgpt'
}

function latestJsonFromPattern(dir: string, pattern: string) {
  if (!fs.existsSync(dir)) return null
  const files = fs.readdirSync(dir).filter((name) => name.startsWith(pattern))
      ...
      configureServer(server) {
        server.middlewares.use('/api/mission-control/system-check', async (req, res, next) => {
          ...
          sendJson(res, code === 0 ? 200 : 500, {
            ...
            exitCode: code,
            payload: { systemCheck: latest?.systemCheck ?? latest },
          })
        })
        ...
        server.middlewares.use('/api/mission-control/model-scan', async (req, res, next) => {
          ...
          const summary = summaryPath ? safeReadJson(summaryPath) : null
          const fallback = safeReadJson(path.join(modelsRoot, 'model-registry.json'))
          sendJson(res, 200, {
            ok: code === 0,
            action: 'model-scan',
        ...
        server.middlewares.use('/api/mission-control/model-benchmark', async (req, res, next) => {
          ...
          if (latest) {
            payload = { resultsPath: latest, results: safeReadJson(latest), markdownPath: path.join(outputsRoot, 'MODEL-BENCHMARKS.md') }
          }
          sendJson(res, code === 0 ? 200 : 500, {
            ok: code === 0,
        ...
        server.middlewares.use('/api/mission-control/open-provider', async (req, res, next) => {
          ...
          const { code, output, error } = await runPowershell('open-browser-provider.ps1', ['-Provider', providerCode, '-Prompt', `"${prompt}"`])
          sendJson(res, 200, {
            ok: code === 0,
            action: 'open-provider',
            command: `powershell -File open-browser-provider.ps1 -Provider ${providerCode}`,
            output: [output, error].filter(Boolean).join('\n'),
            exitCode: code,
        ...
        server.middlewares.use('/api/mission-control/save-ticket', async (req, res, next) => {
          ...
          if (!ticket || !ticket.id) {
            return sendJson(res, 400, { ok: false, action: 'save-ticket', output: '', error: 'ticket.id is required' })
          }
          if (!fs.existsSync(ticketsRoot)) fs.mkdirSync(ticketsRoot, { recursive: true })
          const fileName = `${ticket.id}.json`
          const filePath = path.join(ticketsRoot, sanitizeFileName(fileName))
        ...
        server.middlewares.use('/api/mission-control/repos', async (req, res, next) => {
          if (req.method !== 'GET') return next()
          if (!fs.existsSync(reposRoot)) return sendJson(res, 200, { ok: true, action: 'repos', output: '', payload: { repos: [] } })
          const repoDirs = fs
            .readdirSync(reposRoot, { withFileTypes: true })
            .filter((item) => item.isDirectory())
            .map((item) => path.join(reposRoot, item.name))
          sendJson(res, 200, { ok: true, action: 'repos', output: '', payload: { repos: repoDirs } })
        })
        ...
        server.middlewares.use('/api/mission-control/repo-action', async (req, res, next) => {
          ...
          const scripts = parseRepoModels(repoPath)
          let command = ''
          let commandOutput = ''
          let exitCode = 0

          if (!repoPath || !fs.existsSync(repoPath)) {
            return sendJson(res, 400, { ok: false, action: 'repo-action', output: '', error: 'Valid repoPath is required' })
          }
          ...
          const run = await runPowershell('git-safe.ps1', ['-Action', 'status', '-Repo', `"${repoPath}"`, '-NoPrompt'])
            ...
            if (!scripts?.build) {
              return sendJson(res, 200, { ok: false, action: 'repo-action', output: 'No build script found', error: 'Missing npm build script in package.json' })
            }
            ...
            if (!scripts?.test) {
              return sendJson(res, 200, { ok: false, action: 'repo-action', output: 'No test script', error: 'Missing npm test script in package.json' })
            }
            ...
            const name = `${path.basename(repoPath)}-${stamp}`
        ...
        server.middlewares.use('/api/mission-control/model-inventory', async (req, res, next) => {
          if (req.method !== 'GET') return next()
          const records = readModelRecords()
          const modelPlanPath = path.join(outputsRoot, 'MODEL-CLEANUP-PLAN.md')
          sendJson(res, 200, {
            ok: true,
            action: 'model-inventory',
            output: 'model registry loaded',
            payload: { records, planPath: modelPlanPath },
          })
        })
      },
    },
